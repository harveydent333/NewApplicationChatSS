...
connection.on("ReceiveMessage", function (messageInfoJSON) {
    ...
    if (document.querySelector('#current-room-id').value == messageInfo['roomId']) {
        if (document.querySelector('#user-name').value == messageInfo['userName']) {
...
// Caller    //user rename {oldUserName}||{newUserName}
connection.on("UserRenameClient", function (newUserName, command) {
    document.getElementById("user-name").value = newUserName;
    createIncomigElemMessage(JSON.parse(command), true);
});
...
function sendCommandHelp(userName, roomId, message) {
    connection.invoke("ReceivingHelpCommand", userName, roomId, message).catch(function (err) {
        return console.error(err.toString());
    });
}
...
function createIncomigElemMessage(messageInfo, isCommand = false, isInfo = false, countRows = 0) {
    if (isCommand || isInfo) {
        ...
        addInfoForLastMessage(messageInfo);
        var loginMessageSender = messageInfo['userName'];
